...
const REPEATED_DESTINATION_THRESHOLD = 20;
const PORT_SCAN_THRESHOLD = 12;
const EXFIL_BYTES_THRESHOLD = 50 * 1024 * 1024;
...
export class NetworkAuditor {
  ...
  async audit() {
    ...
    annotateAnomalies(events);
    return {
      schemaVersion: 1, mode: 'audit', startedAt, completedAt: this.now().toISOString(),
      summary: { connections: events.length, suspicious: events.filter(x => x.verdict === 'suspicious').length, unsignedProcesses: events.filter(x => x.signature.status !== 'valid').length, anomalous: events.filter(x => x.anomalies.length > 0).length, repeatedDestinations: events.filter(x => x.anomalies.includes('repeated-destination')).length, portScanPatterns: events.filter(x => x.anomalies.includes('port-scan-pattern')).length, probableExfiltration: events.filter(x => x.anomalies.includes('probable-exfiltration')).length, truncated: allEvents.length > this.maxEvents },
...
export async function writeNetworkReport(directory, report) {
  ...
  const rows = [['timestamp','verdict','reason','protocol','remoteAddress','remotePort','domain','processId','processName','processPath','signatureStatus','publisher','bytesSent','bytesReceived','anomalySeverity','anomalies']];
  for (const event of report.events) rows.push([report.completedAt,event.verdict,event.explanation,event.protocol,event.remoteAddress,event.remotePort,event.domain,event.process.id,event.process.name,event.process.path,event.signature.status,event.signature.publisher,event.bytesSent,event.bytesReceived,event.anomalySeverity,event.anomalies?.join(';')]);
...
function explainEvent(raw, indicators) {
  ...
  return {
    ...
    bytesSent: bytes(raw?.BytesSent), bytesReceived: bytes(raw?.BytesReceived), anomalies: [], anomalySeverity: 'info',
...
function annotateAnomalies(events) {
  const byDestination = new Map(); const byProcessDestination = new Map(); const bytesByProcess = new Map();
  for (const event of events) {
    const destinationKey = event.remoteAddress || event.domain ? `${event.remoteAddress || event.domain}:${event.remotePort || 0}` : '';
    if (destinationKey) byDestination.set(destinationKey, (byDestination.get(destinationKey) ?? 0) + 1);
    const processKey = `${event.process.id}:${event.remoteAddress}`;
    const ports = byProcessDestination.get(processKey) ?? new Set(); ports.add(event.remotePort); byProcessDestination.set(processKey, ports);
    bytesByProcess.set(event.process.id, (bytesByProcess.get(event.process.id) ?? 0) + event.bytesSent);
  }
  for (const event of events) {
    const anomalies = [];
    const destinationKey = event.remoteAddress || event.domain ? `${event.remoteAddress || event.domain}:${event.remotePort || 0}` : '';
    if (destinationKey && (byDestination.get(destinationKey) ?? 0) >= REPEATED_DESTINATION_THRESHOLD) anomalies.push('repeated-destination');
    const ports = byProcessDestination.get(`${event.process.id}:${event.remoteAddress}`);
    if (ports && ports.size >= PORT_SCAN_THRESHOLD) anomalies.push('port-scan-pattern');
    if ((bytesByProcess.get(event.process.id) ?? 0) >= EXFIL_BYTES_THRESHOLD) anomalies.push('probable-exfiltration');
    event.anomalies = anomalies;
    event.anomalySeverity = anomalies.length ? anomalies.includes('probable-exfiltration') || anomalies.includes('port-scan-pattern') ? 'medium' : 'low' : 'info';
    if (anomalies.length) event.explanation = `${event.explanation} Patrón anómalo observado: ${anomalies.map(labelAnomaly).join(', ')}; requiere revisión contextual.`;
  }
}
function labelAnomaly(value) { return ({'repeated-destination':'destino repetido','port-scan-pattern':'variación de puertos','probable-exfiltration':'volumen saliente elevado'})[value] ?? value; }
function bytes(value) { const number = Number(value); return Number.isFinite(number) && number >= 0 ? Math.min(Math.trunc(number), Number.MAX_SAFE_INTEGER) : 0; }
...
function unavailable(at,error) { return {schemaVersion:1,mode:'audit',startedAt:at,completedAt:at,error,summary:{connections:0,suspicious:0,unsignedProcesses:0,anomalous:0,repeatedDestinations:0,portScanPatterns:0,probableExfiltration:0,truncated:false},windowsSecurity:{firewall:[],defender:null},events:[]}; }
