...
export function validateDefinitions(definitions) {
  ...
  if (definitions.threatIntel !== undefined) validateThreatIntel(definitions.threatIntel);
...
function validateThreatIntel(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value) || value.schemaVersion !== 1) throw new Error('Threat intelligence metadata is invalid');
  if (!Number.isSafeInteger(value.retentionDays) || value.retentionDays < 1 || value.retentionDays > 366) throw new Error('Threat intelligence retention is invalid');
  if (!Number.isFinite(Date.parse(value.generatedAt))) throw new Error('Threat intelligence timestamp is invalid');
  if (!value.entries || typeof value.entries !== 'object' || Array.isArray(value.entries) || Object.keys(value.entries).length > 250_000) throw new Error('Threat intelligence entries are invalid');
  for (const [hash, entry] of Object.entries(value.entries)) {
    if (!/^[0-9a-f]{64}$/i.test(hash) || !entry || typeof entry !== 'object' || Array.isArray(entry)) throw new Error('Threat intelligence hash entry is invalid');
    for (const field of ['providers', 'families', 'tags', 'references']) {
      if (!Array.isArray(entry[field]) || entry[field].length > (field === 'references' ? 4 : field === 'providers' ? 8 : 16) || entry[field].some(item => typeof item !== 'string' || item.length > (field === 'references' ? 500 : field === 'providers' ? 40 : field === 'families' ? 160 : 80))) throw new Error(`Threat intelligence ${field} are invalid`);
    }
    if (!Number.isFinite(Number(entry.confidence)) || Number(entry.confidence) < 0 || Number(entry.confidence) > 100) throw new Error('Threat intelligence confidence is invalid');
    for (const field of ['firstSeen', 'lastSeen', 'expiresAt']) {
      if (entry[field] !== null && !Number.isFinite(Date.parse(entry[field]))) throw new Error(`Threat intelligence ${field} is invalid`);
    }
  }
}
